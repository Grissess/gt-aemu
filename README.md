...
If the Python emulator is too slow (it usually is), there's a port of the
MOS6502 emulator straight from [the emulator][emu] included in the `cpp`
directory. It should be stdlib compatible; assuming you have C++ build tools
(on Ubuntu/Debian: `apt install build-essential`, for example), you should be
able to:

```
cd cpp
make
./emu romfile scriptfile
```

This EXPERIMENTAL emulator _only_ supports scriptfiles, and does not have any
options so far. It writes U8 samples to stdout; if you want to play them, you
probably have to capture them:

```
./emu romfile scriptfile > samplefile
aplay -f U8 -c 1 -r 44192 samplefile
```

Or you can use a pipe and cut out the file:

```
./emu romfile scriptfile | aplay -f U8 -c 1 -r 44192
```

Note that the sample rate here is _especially_ arbitrary, since no statistics
are (yet) generated by this emulator. It's up to you to interpret the ROM's
_expected_ period between samples; this emulator merely writes the samples to
stdout as they stream in.

[emu]: https://github.com/clydeshaffer/GameTankEmulator
...
3. Firmware that writes to `$8000` _multiple times_ within the same IRQ window
   will confuse this emulator (each one will look like a separate sample,
   rather than the hardware-faithful behavior of latching this value until the
   next IRQ). Most firmwares shouldn't do this; it's onerous to write there as
   opposed to the aliased zero-page $00 address.
...
[audio]: https://wiki.gametank.zone/doku.php?id=hardware:audio
